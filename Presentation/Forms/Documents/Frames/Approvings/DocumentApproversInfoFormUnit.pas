...
type
  ...
  TDocumentApproversInfoForm = class(TForm)
    ...
    DocumentApprovingsTreeList: TcxDBTreeList;
    ...
    procedure DocumentApprovingsTreeListDblClick(Sender: TObject);
    procedure DocumentApprovingsTreeListFocusedNodeChanged(
      Sender: TcxCustomTreeList; APrevFocusedNode,
      AFocusedNode: TcxTreeListNode);
    procedure DocumentApprovingsTreeListCustomDrawDataCell(
  ...
  protected
    ...
    procedure SetTableColumnLayout(Value: TDocumentApprovingSetHolder);
  ...
  protected
    ...
    function GetAddingApproversOptionsVisible: Boolean;
    function GetRemovingApproversOptionsVisible: Boolean;
    procedure SetAddingApproversOptionsVisible(const Value: Boolean);
    procedure SetRemovingApproversOptionsVisible(const Value: Boolean);

    function GetApprovingsControlPanelVisible: Boolean;
    procedure SetApprovingsControlPanelVisible(const Value: Boolean);

    procedure UpdateApproversControlPanelVisibility;
  ...
  public
    ...
    property AddingApproversOptionsVisible: Boolean
    read GetAddingApproversOptionsVisible
    write SetAddingApproversOptionsVisible;

    property RemovingApproversOptionsVisible: Boolean
    read GetRemovingApproversOptionsVisible
    write SetRemovingApproversOptionsVisible;

    property ApprovingsControlPanelVisible: Boolean
    read GetApprovingsControlPanelVisible
    write SetApprovingsControlPanelVisible;
...
begin
  ...
  with EmployeeSetHolder do begin
    ...
    try
      ...
      DocumentApprovingSetHolder.DisableControls;
...
begin
  ...
  DocumentApprovingsTreeList.BeginUpdate;
...
begin
  ...
  try
    ...
    {
    for I := 0 to DocumentApprovingsTreeList.AbsoluteVisibleCount - 1 do begin

       ApprovingNode :=
        DocumentApprovingsTreeList.AbsoluteVisibleItems[I] as
        TcxDBTreeListNode;
...
begin
  ...
  try
    ...
    for I := 0 to DocumentApprovingsTreeList.SelectionCount - 1 do begin

      SelectedApprovingTreeNode :=
        DocumentApprovingsTreeList.Selections[I] as TcxDBTreeListNode;
...
procedure TDocumentApproversInfoForm.DocumentApprovingsTreeListCustomDrawDataCell(
  Sender: TcxCustomTreeList; ACanvas: TcxCanvas;
  AViewInfo: TcxTreeListEditCellViewInfo; var ADone: Boolean);
begin
...
procedure TDocumentApproversInfoForm.DocumentApprovingsTreeListDblClick(
  Sender: TObject
);
...
begin
  ...
  ApprovingViewModel :=
    CreateDocumentApprovingViewModelFrom(
      DocumentApprovingsTreeList.FocusedNode as TcxDBTreeListNode
    );
...
DocumentApprovingsTreeListFocusedNodeChanged(
...
begin
  ...
  CanBeRemovedColumn :=
    DocumentApprovingsTreeList.GetColumnByFieldName(
      FDocumentApprovingSetHolder.CanBeRemovedFieldName
    );
...
begin
  ...
  DocumentApprovingsTreeList.EndUpdate;
...
begin
  ...
  try
    ...
    DocumentApprovingsTreeList.LayoutChanged;
    
    for I := 0 to DocumentApprovingsTreeList.AbsoluteVisibleCount - 1
    do begin
...
function TDocumentApproversInfoForm.GetAddingApproversOptionsVisible: Boolean;
begin

  Result := AddApproversButton.Visible;
...
function TDocumentApproversInfoForm.GetApprovingsControlPanelVisible: Boolean;
begin

  Result := DocumentApproversControlPanel.Visible;
...
begin
  ...
  Result :=
    DocumentApprovingsTreeList.GetColumnByFieldName(FieldName);
...
begin
  ...
  Result := TcxDBTreeListColumn(DocumentApprovingsTreeList.FocusedColumn);
...
function TDocumentApproversInfoForm.GetRemovingApproversOptionsVisible: Boolean;
begin

  Result := RemoveApproversButton.Visible;
...
begin
  ...
  PerformingResultId :=
    AViewInfo.Node.Values[ApprovingPerformingResultIdColumn.ItemIndex];

  if VarIsNull(IsApprovingAccessible) or (not IsApprovingAccessible)
  then Exit;
  
  if PerformingResultId = DocumentApprovingSetHolder.PerformingResultIsNotPerformedValue
  then
...
begin
  ...
  if DocumentApprovingsTreeList.SelectionCount = 0 then Exit;
...
procedure TDocumentApproversInfoForm.SetAddingApproversOptionsVisible(
  const Value: Boolean);
begin

  AddApproversButton.Visible := Value;

  UpdateApproversControlPanelVisibility;

end;

procedure TDocumentApproversInfoForm.SetApprovingsControlPanelVisible(
  const Value: Boolean);
begin

  DocumentApproversControlPanel.Visible := Value;
...
procedure TDocumentApproversInfoForm.SetRemovingApproversOptionsVisible(
  const Value: Boolean);
begin

  RemoveApproversButton.Visible := Value;

  UpdateApproversControlPanelVisibility;
  
end;

procedure TDocumentApproversInfoForm.SetTableColumnLayout(
  Value: TDocumentApprovingSetHolder);
begin

  with Value.FieldDefs do begin

    ApproverNameColumn.DataBinding.FieldName := PerformerNameFieldName;
    RecordIdColumn.DataBinding.FieldName := IdFieldName;
    ApproverIdColumn.DataBinding.FieldName := PerformerIdFieldName;
    ApproverSpecialityColumn.DataBinding.FieldName := PerformerSpecialityFieldName;
    ApproverDepartmentNameColumn.DataBinding.FieldName := PerformerDepartmentNameFieldName;
    ApprovingPerformingResultIdColumn.DataBinding.FieldName := PerformingResultIdFieldName;
    ApprovingPerformingResultColumn.DataBinding.FieldName := PerformingResultFieldName;
    ApprovingPerformingDateColumn.DataBinding.FieldName := PerformingDateTimeFieldName;
    ActuallyPerformedEmployeeIdColumn.DataBinding.FieldName := ActuallyPerformedEmployeeIdFieldName;
    ActuallyPerformedEmployeeColumn.DataBinding.FieldName := ActuallyPerformedEmployeeNameFieldName;
    NoteColumn.DataBinding.FieldName := NoteFieldName;
    IsViewedByApproverColumn.DataBinding.FieldName := IsViewedByPerformerFieldName;
    TopLevelRecordIdColumn.DataBinding.FieldName := TopLevelApprovingIdFieldName;
    ...
    IsApprovingAccessibleColumn.DataBinding.FieldName := IsApprovingAccessibleFieldName;

    DocumentApprovingsTreeList.DataController.KeyField := IdFieldName;
    DocumentApprovingsTreeList.DataController.ParentField := IdFieldName;
...
begin
  ...
  DocumentApproversDataSource.DataSet := FDocumentApprovingSetHolder.DocumentApprovingSet;
...
begin
  ...
  if Value then begin
    ...
    DocumentApprovingsTreeList.PopupMenu := nil;
    ...
    DocumentApprovingsTreeList.PopupMenu := ApproversControlPopupMenu;

    DocumentApproversControlPanel.Align := alTop;
    
  end;

  DocumentApprovingsTreeList.OptionsData.Editing := not Value;
...
procedure TDocumentApproversInfoForm.UpdateApproversControlPanelVisibility;
begin

  DocumentApproversControlPanel.Visible :=
    AddApproversButton.Visible and RemoveApproversButton.Visible;

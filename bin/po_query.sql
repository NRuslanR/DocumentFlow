select distinct doc.id as document_id,
doc.id as base_document_id,doc.type_id as document_kind_id,
...
end as document_charge_sheet_id,
case when doc_rec.issuer_id is not null then
doc_rec.document_id else null
end as charge_sheet_document_id,
...
doc_rec.top_level_charge_sheet_id
else null
end as top_level_document_charge_sheet_id,
case when doc_rec.issuer_id is not null then
doc_rec.charge
else null
end as document_charge_sheet_text,
case when doc_rec.issuer_id is not null then
doc_rec.comment
else null
end as document_charge_sheet_response,
case when doc_rec.issuer_id is not null then
doc_rec.charge_period_start
else null
end as document_charge_sheet_period_start,
case when doc_rec.issuer_id is not null then
doc_rec.charge_period_end
else null
end as document_charge_sheet_period_end,
case when doc_rec.issuer_id is not null then
doc_rec.performing_date
else null
end as document_charge_sheet_performing_date,
case when doc_rec.issuer_id is not null then doc_rec.issuing_datetime else null end as document_charge_sheet_issuing_datetime,case when doc_rec.issuer_id is not null then
doc_rec.is_for_acquaitance
else null
end as document_charge_sheet_is_for_acquaitance,
case when doc_rec.issuer_id is not null then
(select role_id from doc.employees_roles where employee_id = performers.id)
else null
end as document_charge_sheet_performer_role_id,case when doc_rec.issuer_id is not null then
performers.id
else null
end as document_charge_sheet_performer_id,
case when doc_rec.issuer_id is not null then
performers.leader_id 
else null
end as document_charge_sheet_perf_leader_id,
case when doc_rec.issuer_id is not null then
performers.is_foreign 
else null
end as document_charge_sheet_perf_is_foreign,
case when doc_rec.issuer_id is not null then
(performers.surname || ' ' || performers.name || ' ' || performers.patronymic)
else null
end as document_charge_sheet_performer_name,
case when doc_rec.issuer_id is not null then
performers.speciality
else null
end as document_charge_sheet_performer_speciality,
case when doc_rec.issuer_id is not null then
perf_deps.id 
else null
end as document_charge_sheet_perf_dep_id,
case when doc_rec.issuer_id is not null then
perf_deps.code 
else null
end as document_charge_sheet_perf_dep_code,
case when doc_rec.issuer_id is not null then
perf_deps.short_name
else null
end as document_charge_sheet_perf_dep_name,

case when doc_rec.issuer_id is not null then
fact_perf.id
else null
end as document_charge_sheet_fact_perf_id,
case when doc_rec.issuer_id is not null then
fact_perf.leader_id
else null
end as document_charge_sheet_fact_perf_leader_id,
case when doc_rec.issuer_id is not null then
fact_perf.is_foreign 
else null
end as document_charge_sheet_fact_perf_is_foreign,
case when doc_rec.issuer_id is not null then
(fact_perf.surname || ' ' || fact_perf.name || ' ' || fact_perf.patronymic)
else null
end as document_charge_sheet_fact_perf_name,
case when doc_rec.issuer_id is not null then
fact_perf.speciality
else null
end as document_charge_sheet_fact_perf_speciality,
case when doc_rec.issuer_id is not null then
fact_perf_dep.id
else null
end as document_charge_sheet_fact_perf_dep_id,
case when doc_rec.issuer_id is not null then
fact_perf_dep.code
else null
end as document_charge_sheet_fact_perf_dep_code,
case when doc_rec.issuer_id is not null then
fact_perf_dep.short_name
else null
end as document_charge_sheet_fact_perf_dep_name,replacings.is_cur_emp_issuer or replacings.is_cur_emp_performer as can_charge_sheet_view,replacings.is_cur_emp_issuer as has_charge_section_access,replacings.is_cur_emp_performer as has_response_section_access,replacings.is_cur_emp_issuer as can_charge_sheet_remove,replacings.is_cur_emp_performer as can_charge_sheet_perform,case when doc_rec.issuer_id is not null then
(select 
min(look_date) from doc.looked_personnel_orders lsn
where
lsn.document_id=doc_rec.document_id and
...
end as document_charge_sheet_viewing_date,fact_charge_sheet_sender.id as charge_sheet_sender_id,
...
doc_fm.id as document_file_id,
doc_fm.file_name as document_file_name,
doc_fm.file_path as document_file_path,
doc_links.id as document_relation_id,
coalesce(rel_docs.id, rel_servs.id, rel_po.id) as related_document_id,coalesce(rel_docs.type_id, rel_servs.type_id, rel_po.type_id) as related_document_kind_id,coalesce(rel_doc_type.single_full_name, rel_serv_type.single_full_name, rel_po_type.single_full_name) as related_document_kind_name,coalesce(rel_docs.document_number, rel_servs.document_number, rel_po.document_number) as related_document_number,coalesce(rel_docs.name, rel_servs.name, rel_po.name) as related_document_name,coalesce(rel_docs.document_date, rel_servs.document_date, rel_po.document_date) as related_document_date,da.id as approving_id,
da.performing_date as approving_performing_date,
da.performing_result_id as approving_performing_result_id,
(approver.id = cur_emp.id or doc.is_employee_acting_for_other_or_vice_versa(cur_emp.id, approver.id)) as document_approving_accessible, dar.result_name as approving_performing_result,
da.note as approving_note,
da.cycle_number as approving_cycle_number,
da.cycle_number as approving_cycle_id,case when da.id is not null then da.cycle_number is not null else null end as approving_is_completed,exists (
select 1
from doc.looked_personnel_orders lsn
where
lsn.document_id=doc.id and
((lsn.looked_employee_id=da.approver_id) or
(doc.is_employee_acting_for_other_or_vice_versa(lsn.looked_employee_id,da.approver_id)))
) as approving_is_looked_by_approver,approver.id as approver_id,
...
fact_approver_dep.short_name as fact_approver_dep_name,doc.sub_type_id as sub_type_id,posk.name as sub_type_name from doc.personnel_orders doc
...
left join doc.personnel_order_charges doc_rec on doc_rec.document_id = doc.id
join doc.document_charge_types dct on dct.id = doc_rec.kind_id left join doc.employees performers on performers.id = doc_rec.performer_id
...
left join doc.personnel_order_signings doc_signings on doc_signings.document_id = doc.id
...
left join doc.personnel_order_approvings da on da.document_id = doc.id
...
left join doc.departments fact_approver_dep on fact_approver_dep.id = fact_approver.department_id left join doc.personnel_order_links doc_links on doc_links.document_id = doc.id
join lateral (select doc.is_employee_acting_for_other_or_vice_versa(cur_emp.id, doc_rec.issuer_id) as is_cur_emp_issuer,doc.is_employee_acting_for_other_or_vice_versa(cur_emp.id, doc_rec.performer_id) as is_cur_emp_performer) as replacings on true left join lateral
(
select
...
left join lateral (select doc_links.related_document_id as id,doc_links.related_document_type_id as type_id,rel_po.document_number,rel_po.name,rel_po.document_date from doc.personnel_orders rel_po where rel_po.id = doc_links.related_document_id and rel_po.type_id = doc_links.related_document_type_id) rel_po on rel_po.id = doc_links.related_document_id left join doc.document_types rel_doc_type on rel_doc_type.id = rel_docs.type_id
left join doc.document_types rel_serv_type on rel_serv_type.id = rel_servs.type_id
left join doc.document_types rel_po_type on rel_po_type.id = rel_po.type_id left join doc.personnel_order_file_metadata doc_fm on doc_fm.document_id = doc.id join doc.personnel_order_sub_kinds posk on posk.id = doc.sub_type_id where doc.id=:pdocument_id

...
case when doc_rec.issuer_id is not null then
(select doc.is_employee_acting_for_other_or_vice_versa(doc.get_current_employee_id(), performers.id))
else null
end as document_charge_sheet_is_accessible_charge,
...
end as document_charge_sheet_fact_perf_dep_name,
case when doc_rec.issuer_id is not null then
(select 
min(look_date) from doc.looked_service_notes lsn
where
...
fact_approver_dep.short_name as fact_approver_dep_name from doc.service_notes doc
...
left join doc.service_note_receivers doc_rec on doc_rec.document_id = doc.id
...
left join doc.service_note_approvings da on da.document_id = doc.id
...
left join doc.departments fact_approver_dep on fact_approver_dep.id = fact_approver.department_id left join doc.service_note_links doc_links on doc_links.document_id = doc.id
left join lateral
(
select
...
left join doc.document_types rel_po_type on rel_po_type.id = rel_po.type_id left join doc.service_note_file_metadata doc_fm on doc_fm.document_id = doc.id where doc.id=:pdocument_id

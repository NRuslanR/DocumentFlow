...
select  
    ...
    where lsn.document_id = a.id and lsn.looked_employee_id = 1355  
    ...
    case when performing_stats.total_charge_count = 0 and performing_stats.performed_charge_count = 0 then null 
    else (performing_stats.performed_charge_count || ' из ' || performing_stats.total_charge_count)::varchar end as charges_stat
    ...
    join doc.get_service_note_performing_statistics(case when a.current_work_cycle_stage_id = 4 then a.id else null end) performing_stats on true 
    ...
    or  
    (  
    doc.is_employee_workspace_includes_other_employee(1355 , a.author_id)  
    and  
    case  
    ...
    or doc.is_employee_replacing_for_other(1355 , a.author_id)  
    )) 
    or  
    ( 
    ...
    and  
    (s.signer_id = 1355  or 
     doc.is_employee_workspace_includes_other_employee(1355 , s.signer_id) or  
     doc.is_employee_replacing_for_other(1355 , s.signer_id)) 
    ) 
    )
),
new as (
